...
module Model =
    ...
    /// Returns the pre-condition arcs of a model.
    let pre (model: Model<'Place, 'Transition>) : Arcs<'Place, 'Transition> = model.Pre

    /// Returns the post-condition arcs of a model.
    let post (model: Model<'Place, 'Transition>) : Arcs<'Place, 'Transition> = model.Post
