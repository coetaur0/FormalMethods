...
let shuffle (list: 'a list) : 'a list =
    ...
    List.sortBy (fun _ -> rnd.Next(1, length)) list
...
module Model =
    ...
    /// Simulates the execution of a model for a given number of steps.
    let rec simulate (model: Model<'Place, 'Transition>) (marking: Marking<'Place>) (steps: int) : Marking<'Place> =
        if steps <= 0 then
            marking
        else
            let nextMarking =
                ...
                |> shuffle
                |> List.tryPick (fun t ->
                    let m = (fire model marking t).Value

                    match Map.forall (fun _ tokens -> tokens >= 0) m.Counts with
                    | true -> Some m
                    | false -> None)

            match nextMarking with
            | Some m -> simulate model m (steps - 1)
            | None -> marking
