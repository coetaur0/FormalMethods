...
/// Checks if a transition is alive in a model starting from some initial marking.
let isTransitionAlive (model: Model<'Place, 'Transition>) (marking: Marking<'Place>) (transition: 'Transition) : bool =
...
/// Checks if a model is alive starting from some initial marking.
let isAlive (model: Model<'Place, 'Transition>) (marking: Marking<'Place>) : bool =
    model
    |> Model.transitions
    |> Set.forall (isTransitionAlive model marking)

/// Checks if a model is blockable starting from some initial marking.
let isBlockable (model: Model<'Place, 'Transition>) (marking: Marking<'Place>) : bool =
    let graph = MarkingGraph.make model marking

    graph
    |> MarkingGraph.markings
    |> Set.exists (fun marking -> Map.isEmpty (Map.find marking graph.Edges))

/// Checks if a model is reversible starting from some initial marking.
let isReversible (model: Model<'Place, 'Transition>) (marking: Marking<'Place>) : bool =
    let graph = MarkingGraph.make model marking

    graph
    |> MarkingGraph.markings
    |> Set.forall (fun marking -> (MarkingGraph.successors graph marking) = (MarkingGraph.markings graph))
